...
Это подход, при котором система подробно проектируется заранее, ещё до начала основной реализации. Обычно заранее продумываются почти все сущности, связи, сценарии, расширения и технические детали.
### Использование в проекте
Не использовался.
### Почему
Для данной ВКР (в случае специфики предмета) такой подход был бы избыточным. Проект небольшой, учебный, и в процессе работы структура всё равно уточнялась: дополнялись сущности БД, менялись связи между компонентами, уточнялся основной сценарий адаптивного тестирования.
### Почему не будем использовать
Для небольшого учебного проекта BDUF не даёт ощутимой пользы, но увеличивает объём подготовительной работы (КПД для студента маленький). В такой задаче разумнее сначала определить основу системы, а детали дорабатывать по мере реализации. Поэтому от полноценного BDUF было решено отказаться.

## SoC (Separation of Concerns) - принцип разделения ответственности
Принцип разделения ответственности, при котором разные части системы отвечают за разные задачи.
### Использование в проекте
Использовался.
### Почему
Этот принцип хорошо подходит для нашей системы и реально был применён в архитектуре:
- клиентская часть отвечает за интерфейс;

- серверная часть отвечает за обработку запросов;

-  проверки отвечает за проверку ответов;

- модуль аналитики отвечает за анализ результатов;

- адаптивный сервис отвечает за формирование рекомендаций и дополнительных тестов;

- база данных отвечает только за хранение данных.

Также разделение видно и на уровне кода:
- контроллеры принимают запросы;

- сервисы содержат бизнес-логику;

- репозитории или слой доступа к данным работают с БД.

### Почему будем использовать
Этот принцип делает проект понятнее и чище. Благодаря ему проще объяснить архитектуру, проще менять отдельные части системы и проще показывать соответствие SOLID. Для такой работы SoC действительно полезен, поэтому он был использован осознанно и целенаправленно.

## MVP (Minimum Viable Product) - минимально жизнеспособный продукт
Это минимально жизнеспособный продукт, в котором реализован только основной, необходимый функционал без лишних дополнений.
...
### Почему
Фактически весь проект и был собран как MVP. В систему включён только тот функционал, который нужен для демонстрации идеи.
В рамках нашего предмета, то что мы уже сделали является MVP. Но в рамках ВКР - до минимальной жизнеспособности ещё далеко.

### Почему будем использовать
Для лабораторной работы это самый правильный подход. Он позволяет показать работающую систему без перегрузки ненужными функциями. Поэтому MVP здесь не просто уместен, а фактически является основой всей реализации. Так же и для последующего использования этого метода для ВКР.
...
### Почему
Мы не делали отдельный прототип только ради проверки идеи. Вместо этого сразу делался минимальный рабочий вариант системы.

### Почему не будем использовать 
Для такого небольшого проекта отдельный PoC не нужен. Идея адаптивного тестирования сама по себе не настолько рискованная технически, чтобы сначала отдельно доказывать её реализуемость. Гораздо рациональнее сразу делать MVP. Поэтому отдельный PoC в проект не включался.
Но для ВКР можно задуматься над использованием данной методологии для сервиса адаптивного тестирования, если будем использовать онтологии.
